...
def parseArguments():
    ...
    parser.add_argument('--remove_limit', default=False, action='store_true', help="Remove the rate limiter to run "
                                                                                   "at max capacity")
    parser.add_argument('--template', default=None, help="Use the template for the document. Provide the path to the "
                                                         "template json document")
    parser.add_argument('--update_template_fields', default=None, help="Comma separated fields to be updated in the "
                                                                       "document. Nested fields should be dot (.)"
                                                                       "separated "
                                                                       "(for example \"Address.City\"). Only the "
                                                                       "valued provided in this list will be updated "
                                                                       "in document. By default all fields that can "
                                                                       "be changed will be updated")
...
class Document(object):
    ...
    def get_document(self, value, update):
        pass
...
class SimpleDocument(Document):
    def __init__(self, **kwargs):
        ...
        body = {}
        super(SimpleDocument, self).__init__(body)

    def get_document(self, value, update):
        body_length = self.size - str(value).__len__() - "val".__len__() - "update".__len__() - "body".__len__()
        doc_body = "".rjust(body_length, 'a')
        body = {'val': int(value), 'update': update, 'body': doc_body}
        return body


class UserDocument(Document):
    def __init__(self, template, update_fields, **kwargs):
        self.template = template
        self.update_fields = update_fields
        self.parser = Parser(template=self.template)
        document = {}
        super(UserDocument, self).__init__(document)

    def get_document(self, value, update):
        self.body = self.parser.parse(value, 0)
        if update == 0:
            return self.body
        updated_document = self.parser.parse(value, update)
        ...
        for field in update_fields:
            ...
            if fields.__len__() > 1:
                for _f in fields[:-1]:
                    current_field = self.body[_f]
                    update_field = updated_document[_f]
                current_field[fields[-1]] = update_field[fields[-1]]
            else:
                self.body[fields[0]] = updated_document[fields[0]]
        return self.body
...
class Parser:
    ...
    {{ int:rand_values(*args) }}
    {{ int:count }}
    {{ bool: rand }}
    {{ float:rand_range(min,max) }}
    {{ float:rand_values(*args) }}
    {{ string:repeat_length(len) }}
    {{ string:repeat_rand_length(minlen, maxlen) }}
    {{ string:rand_char_length(length) }}
    {{ string:rand_char_rand_length(minlen, maxlen) }}
    {{ string:rand_values(*args) }}
    {{ repeat {{ template }} fix_len(count) }}
    {{ repeat {{ template }} rand_len(min, max) }}
    """

    def jinja2_parser(self, template):
        first_strip = template.partition("{{")[2].rpartition("}}")[0].strip()
        if first_strip.startswith('int'):
            if "rand_range" in first_strip:
                range = first_strip.partition('(')[2].rpartition(')')[0].split(',')
                min = int(range[0])
                max = int(range[1])
                return self.random.randint(min - 1, max)
            elif "rand_values" in first_strip:
                values = first_strip.partition('(')[2].rpartition(')')[0].split(',')
                choices = []
                for value in values:
                    choices.append(int(value))
                return self.random.choice(choices)
            elif "count" in first_strip:
                return self.count
            else:
                return first_strip
        if first_strip.startswith('bool'):
            return self.random.choice([True, False])
        if first_strip.startswith('float'):
            if "rand_range" in first_strip:
                range = first_strip.partition('(')[2].rpartition(')')[0].split(',')
                min = int(range[0])
                max = int(range[1])
                ...
                return first_strip
        if first_strip.startswith('string'):
            if "repeat_length" in first_strip:
                length = first_strip.partition('(')[2].rpartition(')')[0]
                length = int(length)
                return "".rjust(length, 'a')
            elif "repeat_rand_length" in first_strip:
                range = first_strip.partition('(')[2].rpartition(')')[0].split(',')
                min = int(range[0])
                max = int(range[1])
                length = self.random.randint(min - 1, max)
                return "".rjust(length, 'a')
            elif "rand_char_length" in first_strip:
                length = first_strip.partition('(')[2].rpartition(')')[0]
                length = int(length)
                return_val = []
                choices = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890"
                for i in xrange(length):
                    char = self.random.choice(choices)
                    return_val.append(char)
                return "".join(return_val)
            elif "rand_char_rand_length" in first_strip:
                range = first_strip.partition('(')[2].rpartition(')')[0].split(',')
                min = int(range[0])
                max = int(range[1])
                length = self.random.randint(min - 1, max)
                ...
                return first_strip
        if first_strip.startswith('repeat'):
            return_val = []
            inner_template = first_strip.partition('{{')[2].rpartition('}}')[0].strip()
            inner_template = "{{ " + inner_template + " }}"
            if "fix_len" in first_strip:
                length = first_strip.partition('fix_len(')[2].rpartition(')')[0]
                length = int(length)
            elif "rand_len" in first_strip:
                range = first_strip.partition('rand_len(')[2].rpartition(')')[0].split(',')
                min = int(range[0])
                max = int(range[1])
                length = self.random.randint(min - 1, max)
            else:
                return first_strip
            for i in xrange(length):
                item = self.jinja2_parser(inner_template)
                return_val.append(item)
            return return_val

    def parser(self, template, result):
        if isinstance(template, dict):
            for key, value in template.items():
                if isinstance(value, dict):
                    result[key] = {}
                    self.parser(value, result[key])
                    continue
                ...
                if isinstance(value, str) or isinstance(value, unicode):
                    if value.startswith("{{") and value.endswith("}}"):
                        result[key] = self.jinja2_parser(value)
                        continue
                    else:
                        result[key] = value
                        continue
            ...
            for item in template:
                ...
                if isinstance(item, str) or isinstance(item, unicode):
                    if item.startswith("{{") and item.endswith("}}"):
                        insert_item = self.jinja2_parser(item)
                        if isinstance(insert_item, list):
                            result.extend(insert_item)
                        else:
    ...
    def parse(self, count, update):
        self.count = count
        self.update = update
        self.random.seed(self.count)
        if self.update > 0:
            jump_ahead = self.template.__len__() * 1000 * self.update
            self.random.jumpahead(jump_ahead)
        result = {}
        self.parser(self.template, result)
        return result
...
class DocumentGenerator:
    def __init__(self, args):
        ...
        doc_template_path = args.template
        if doc_template_path:
            with open(doc_template_path) as template_file:
                self.template = json.load(template_file)
        else:
            self.template = None
        self.update_template_fields = args.update_template_fields
        self.connections = []
    ...
    def create_connections(self):
        ...
        for i in range(0, self.threads):
            if self.cb_version > '5':
                ...
                bucket = cluster.open_bucket(self.bucket_name)
                bucket.timeout = self.timeout
                self.connections.append(bucket)
            else:
                bucket = Bucket('{0}/{1}'.format(self.spec, self.bucket_name))
                bucket.timeout = self.timeout
                self.connections.append(bucket)

    def create_document_parsers(self):
        """"
        Create document parsers.
        """
        for i in range(0, self.threads):
            if self.template:
                doc_parser = UserDocument(template=self.template, update_fields=self.update_template_fields)
                self.document_parsers.append(doc_parser)
            else:
                doc_parser = SimpleDocument(size=self.size)
                self.document_parsers.append(doc_parser)
    ...
    def get_upsert_items(self, start, end, document_parser):
        """
        Get the upsert items.
        :param document_parser: Document parser object used to create documents
        ...
        for x in range(start, end):
            ...
            document = document_parser.get_document(x, self.current_update_counter)
            items[key] = document
        return items

    def get_retry_upsert_items(self, document_parser):
        """
        ...
        for item in self.retry_batches:
            ...
            document = document_parser.get_document(doc_num, self.current_update_counter)
            items[item] = document
        return items
    ...
    def upsert_thread(self, connection, batch, document_parser):
        items = self.get_upsert_items(batch.start, batch.end, document_parser)
    ...
    def validate_items(self, connection, start, end, document_parser, replica=False):
        """
        Validate items in the bucket
        :param document_parser: The document parser object to create documents
        :param connection: Bucket object for connection
        ...
        for i in range(start, end):
            ...
            document = document_parser.get_document(i, self.current_update_counter)
            value = document
            if key in result:
    ...
    def validate_thread(self, connection, batch, document_parser):
        self.validate_items(connection, batch.start, batch.end, document_parser)
        if self.replicate_to:
            self.validate_items(connection, batch.start, batch.end, document_parser, replica=True)
    ...
    def single_upsert_pass(self):
        ...
        for i in range(0, self.batches.__len__()):
            ...
            document_parser = self.document_parsers[i % num_of_connections_available]
            args.append((connection, self.batches[i], document_parser))
        ...
        while self.retry_batches:
            items = self.get_retry_upsert_items(self.document_parsers[0])
            self.retry_batches = []
